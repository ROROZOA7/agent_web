...
## Better Harness (Pi / Oh My Pi)

Start a new Pi or Oh My Pi session after installing or updating the project
package. Use the host-native review prompt:

```text
/better-harness analyze this project's AI coding workflow and generate an evidence-backed report
```

The portable report is written under `.pi/better-harness/` with paired
`findings.json`, `report.md`, and `report.html`. Review evidence is bounded to
the selected workspace and authorized Pi session source; configured Skills and
packages establish presence, not runtime use. OMP sessions are included when
`PI_CODING_AGENT_DIR` points at the OMP agent directory.

This route reviews the coding-agent workflow. It does not invent application
commands, credentials, product policy, or validation claims. Keep those in
consumer-owned documentation and executable proof.
